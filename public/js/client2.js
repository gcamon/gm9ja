(function() {
	...
	.controller("feedsCtlr",["$scope","DataSource","feedsFactory","$filter","localManager","$interval",
		"$http","$rootScope","ModalService","$filter",
		function($scope,DataSource,feedsFactory,$filter,localManager,$interval,$http,$rootScope,ModalService,$filter){	     
    ...
    var setData = function(type,name,res,featured) { 
    	...
    	var imgSource = (res.data.rss)
    	section++;

    	console.log(imgSource.channel);
    	if(!newsObj[name]) {
    		lst = (imgSource.channel) ? res.data.rss.channel.item : res.data.feed.entry;
    		ft = $filter('orderBy')(lst, 'pubDate')

	    	newsObj[name] = {
	    		title: name ,//imgSource.channel.title || "",
	    		image: (imgSource.channel.image) ? imgSource.channel.image.url : "",
	    		item: ft
	    	}
    	} else {
    		item = (imgSource.channel) ? res.data.rss.channel.item : res.data.feed.entry
    		con = newsObj[name].item.concat(item);
    		$filter('orderBy')(con, "pubDate");
    	}

    	$rootScope.nw = newsObj;
